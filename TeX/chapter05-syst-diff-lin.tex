...
\end{enumerate}
\item Soit $\{u_1+ i \cdot w_1,\dots,u_n + i \cdot w_n\}$ une base de $\C^n$ où $u_i,w_i \in \R^n$  pour tout $i$. Montrer que $\spa\{u_i, w_i \colon 1 \leq i \leq n\} = \R^n$. \label{el:item:5}
\end{enumerate}
...
\begin{displaymath} 
  ...
  \begin{pmatrix} \displaystyle 
    ...
    \displaystyle \sum_{k=0}^{\infty}\dfrac{(-1)^{k}t^{2k+1}}{(2k+1)!} & \displaystyle \sum_{k=0}^{\infty}\dfrac{(-1)^{k}t^{2k}}{(2k)!}
\end{pmatrix}
\end{displaymath}
...
\end{pmatrix} 
\end{displaymath}
est une solution respectant les conditions initiales  $v_0 = 
\begin{pmatrix}
    1\\
    2
\end{pmatrix} $. 
...
\begin{eqnarray}
  ...
  & = &  \displaystyle ∑_{k=0}^{2N}  ∑_{j = 0}^k \binom{k}{j} \frac{x^{k-j} ⋅ y^{j} }{k!}.   
\end{eqnarray}
En fait, c'est le cas pour $x,y ∈ R$, où $R$ est un anneau et $x ⋅ y  = y ⋅x$, c.à.d. $x$ et $y$ commutent.  Dans ce cas, on a aussi
\begin{displaymath}
  \displaystyle  ∑_{k=0}^{2N} \frac{(x+y)^j}{k!} =  \displaystyle  ∑_{k=0}^{2N}  ∑_{j = 0}^k \binom{k}{j} \frac{x^{k-j} y^{j} }{k!}.   
\end{displaymath}
%
...
Il est facile de voir que la matrice représentant le décalage plus $λ$ par rapport à la base canonique de $\mathbb{C}^n$ est
...
\section{Déterminer la forme normale de Jordan}
...
\begin{lemma}
  ...
  \begin{enumerate}[i)]
  \item $T(V_i) ⊆ V_i$ c.à.d. $V_i$ invariant sous $T$ pour tout $i$  et \label{sl:item:10}
...
\end{displaymath}
et avec $V_i = \ker( T - λ_i\id)^{m_i}$ on a $V = V_1 ⊕ \cdots ⊕ V_K$  et \ref{sl:item:10}) avec lemme~\ref{lem:20}. 
...
\subsection{Le polynôme minimal et la diagonalisation}
